...
def main():
    ...
    parser.add_argument('--celltype_annotation_ref',type=str,help='if it is not none, it means to use the reference species dataset to annotate another one',default=None)
    ...
    run = wandb.init(project='esmAAE_llama_2k_2k_task88_final',config=config)
    ...
    # Calculate the number of high-variable genes per species for LLaMA and ESM # note, we can not do this since we may get more hvgs than the specify amount of hvg
    ...
    for i in range(num_species):
        ...
        llama_genes_per_species_target=llama_genes_per_species[species]
        # Slice the weights for the current species
        if i==0:
            llama_start_idx = i * llama_genes_per_species_target
            llama_end_idx = (i + 1) * llama_genes_per_species_target
        else:
        ...
        esm_genes_per_species_target=esm_genes_per_species[species]
        if i ==0:
            esm_start_idx = i * esm_genes_per_species_target
            esm_end_idx = (i + 1) * esm_genes_per_species_target
        else:
        ...
        # save the h5ad file
        macrogene_adata[species].write_h5ad(output_path+'/'+species+'_macrogene_adata.h5ad')
    '''
    ...
    for epoch in range(train_epochs):
        ...
        for batch in dataloader:
            ...
            if celltype_annotation_ref is not None:
                target_mask = (species_idx == 1) # 0 is the reference dataset, 1 is the query
