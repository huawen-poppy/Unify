...
def main():
    ...
    with open('config_for_test_model.yaml') as file:
    ...
    config = {'batch_size':256,
              ...
              'latent_size':1024,
              'macrogene_encoder_dropout_ratio':0.1,
              'macrogene_decoder_dropout_ratio':0.2,
              'lr_generator':0.0012961530059141459,
              'lr_encoder':0.006394972433915013,
              'lr_discriminator_species':0.008353044657473358,
              'lr_discriminator_celltype':0.003681956728050747,
    ...
    run = wandb.init(project='esmAAE_llama_task10_testing_best_parameter_of_pretrain_with_llama100_allGO',config=config)
    llama_macrogene_amount=wandb.config['num_llama_macrogene']
    '''
    #llama_macrogene_amount=wandb.config.num_llama_macrogene
    ...
    '''
    macrogene_encoder_hidden_size=128
    macrogene_decoder_hidden_size=128
    macrogene_encoder_latent_size=128
    macrogene_decoder_latent_size=128
    macrogene_encoder_dropout_ratio=0.1
    ...
    '''
    hidden_size_adversary_species=128
    hidden_size_adversary_celltype=128
    lr_generator=1e-3
    ...
    for epoch in range(train_epochs):
        ...
        if (epoch+1)%10==0:
            overall_celltype_ARI = evaluation_model_embed(encoder,data=dataset)
            ...
            #torch.save(encoder.state_dict(), output_path+'/encoder_pareto-'+str(epoch)+'.pth')
            #torch.save(decoder.state_dict(), output_path+'/decoder_pareto-'+str(epoch)+'.pth')
            #torch.save(discriminator_species.state_dict(),output_path+'/discriminator_species_pareto-'+str(epoch)+'.pth')
            #torch.save(discriminator_celltype.state_dict(), output_path+'/discriminator_celltype_pareto-'+str(epoch)+'.pth')
