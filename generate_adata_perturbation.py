...
def main():
    ...
    # below is to generate the human other cytokine predictions
    mouse_pbs=adata_basal[(adata_basal.obs['species']=='mouse') & (adata_basal.obs['celltype']=='PBS')].obsm['X_esm']
    human_pbs=adata_basal[(adata_basal.obs['species']=='human') & (adata_basal.obs['celltype']=='PBS')].obsm['X_esm']
    all_cytokine=np.unique(adata_basal.obs['celltype']).tolist()
    all_cytokine.remove('PBS')
    all_esm_num=len(all_gene_names_esm)
    human_gene_num_esm=np.sum([i.split("_")[0]=='human' for i in all_gene_names_esm])
    all_llama_num=len(all_gene_names_llama)
    human_gene_num_llama=np.sum([i.split("_")[0]=='human' for i in all_gene_names_llama])
    all_genes_name_esm_human=all_gene_names_esm[0:human_gene_num_esm]
    all_genes_name_llama_human=all_gene_names_llama[0:human_gene_num_llama]
    
    all_genes_name_esm_human=[i.replace('human_','') for i in all_genes_name_esm_human]
    all_genes_name_llama_human=[i.replace('human_','') for i in all_genes_name_llama_human]
    #import pdb;pdb.set_trace()
    human_esm_centroid_weights=esm_centroid_weights[0:human_gene_num_esm,:]
    human_llama_centroid_weights=llama_centroid_weights[0:human_gene_num_llama,:]
    
    esm_W   = torch.tensor(human_esm_centroid_weights)
    llama_W = torch.tensor(human_llama_centroid_weights)

    esm_W_pinv   = torch.pinverse(esm_W)   # shape [D_esm, n_genes]
    llama_W_pinv = torch.pinverse(llama_W)
    

    #import pdb;pdb.set_trace()
    def raw_count_reconstruction(macrogene_recon, esm_W, llama_W,all_genes_name_esm_human,all_genes_name_llama_human,cytokine,save_label):
        esm_recon = macrogene_recon[:,0:2000]
        llama_recon = macrogene_recon[:,2000:]

        Ys = esm_recon.cpu().detach().numpy()
        W = esm_W.cpu().detach().numpy()
        WT = W.T
        λ  = 1e-2
        inv = np.linalg.inv(WT @ W + λ * np.eye(WT.shape[0]))
        ridge_pinv = inv @ WT
        adata_origin_esm_rec = Ys @ ridge_pinv 
        adata_origin_esm_rec = np.clip(adata_origin_esm_rec, 0, None)
        #adata_origin_esm_nonneg = np.rint(adata_origin_esm_rec).astype(int)
        adata_origin_esm_nonneg=adata_origin_esm_rec

        Ys2 = llama_recon.cpu().detach().numpy()
        W2 = llama_W.cpu().detach().numpy()
        WT2 = W2.T
        λ  = 1e-2
        inv2 = np.linalg.inv(WT2 @ W2 + λ * np.eye(WT2.shape[0]))
        ridge_pinv2 = inv2 @ WT2
        adata_origin_llama_rec = Ys2 @ ridge_pinv2 
        adata_origin_llama_rec = np.clip(adata_origin_llama_rec, 0, None)
        #adata_origin_llama_nonneg = np.rint(adata_origin_llama_rec).astype(int)
        adata_origin_llama_nonneg = adata_origin_llama_rec

        # save h5ad
        adata_origin_esm_nonneg_ad=AnnData(adata_origin_esm_nonneg)
        adata_origin_llama_nonneg_ad=AnnData(adata_origin_llama_nonneg)
        adata_origin_esm_nonneg_ad.obs['cytokine']=cytokine
        adata_origin_llama_nonneg_ad.obs['cytokine']=cytokine
        adata_origin_esm_nonneg_ad.var_names=all_genes_name_esm_human
        adata_origin_llama_nonneg_ad.var_names=all_genes_name_llama_human
        adata_origin_esm_nonneg_ad.write_h5ad('/ibex/project/c2101/species_integration/figure6_perturbation/human_pred_esm_ridge_clip_'+save_label+'_'+cytokine+'.h5ad')
        adata_origin_llama_nonneg_ad.write_h5ad('/ibex/project/c2101/species_integration/figure6_perturbation/human_pred_llama_ridge_clip_'+save_label+'_'+cytokine+'.h5ad')


    #target_cytokine=['CD30L', 'Decorin', 'EGF', 'G-CSF', 'GM-CSF', 'IFN-alpha1', 'IGF-1', 'IL-1-beta', 'IL-10', 'IL-12', 'IL-13', 'IL-15', 'IL-19', 'IL-1Ra', 'IL-20', 'IL-24', 'IL-36-alpha', 'IL-6', 'IL-7', 'LIGHT', 'Noggin', 'OX40L', 'TRAIL']
    for cytokine in all_cytokine:
        mouse_cyto=adata_basal[(adata_basal.obs['species']=='mouse') & (adata_basal.obs['celltype']==cytokine)].obsm['X_esm']
        human_cyto_basal=human_pbs+mouse_cyto.mean(axis=0)-mouse_pbs.mean(axis=0)
        input_cells_num=human_cyto_basal.shape[0]
        human_labels= np.tile([1, 0], (input_cells_num, 1))
        combined_input = torch.cat(( torch.tensor(human_cyto_basal), torch.tensor(human_labels)), dim=1)
        
        with torch.no_grad():
            macrogene_recon = finetune_decoder(combined_input.to(device))
        #import pdb;pdb.set_trace()
        #raw_count_reconstruction(macrogene_recon,esm_W, llama_W,all_genes_name_esm_human,all_genes_name_llama_human,cytokine,save_label='recon_basal_diff_pp_input')

        # method 3, in the basal state we use the nn.linear to map the mouse basal to mouse perturb
        lr = LinearRegression(fit_intercept=True)  
        lr.fit(mouse_pbs.mean(axis=0).reshape(1,-1),mouse_cyto.mean(axis=0).reshape(1,-1))
        human_preturb_pred = lr.predict(human_pbs)

        human_preturb_pred_input = torch.cat(( torch.tensor(human_preturb_pred), torch.tensor(human_labels)), dim=1)
        with torch.no_grad():
            macrogene_recon_lr = finetune_decoder(human_preturb_pred_input.to(device))
        raw_count_reconstruction(macrogene_recon_lr,esm_W, llama_W,all_genes_name_esm_human,all_genes_name_llama_human,cytokine,save_label='recon_linear_model_pp_input')
